...
2. **Word（优先路径 — 格式精确）**：使用 **`tools/minimax-docx/build_disclosure.sh`**，基于参考示例模板生成格式精确的 .docx（保持参考示例的样式、字体、字号、公式环境）：

`python3 tools/mermaid_render.py -i <含图示的草稿.md> -o "<案件名_YYYYMMDDHHmmss>.md"`（mermaid 转 PNG）

`bash tools/minimax-docx/build_disclosure.sh --markdown "<案件名_YYYYMMDDHHmmss>.md" --output "<案件名_YYYYMMDDHHmmss>.docx"`

（需 dotnet SDK 8.0+ 及 python-docx；见 `tools/minimax-docx/README.md`）

3. **Word（降级路径）**：若 minimax-docx 不可用，对 .md 执行 `md_to_docx.py`：

`python3 tools/md_to_docx.py -i "<案件名_YYYYMMDDHHmmss>.md" -o "<案件名_YYYYMMDDHHmmss>.docx"`
