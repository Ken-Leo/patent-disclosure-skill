...
**交付策略**：以 `examples/参考示例--一种光盘数据读取过程中的回读信号样本序列检测方法.docx` **为模板**，**直接替换内容**，**不改变任何格式**（字体、字号、行距、段落缩进、分节、页脚、图片框架、公式环境等全部保留）。

**版本与覆盖**：凡交付均以 **§7.3 第 5 点**「**案件名 + 时间戳**」落盘，同目录下多文件即版本历史。

### Word 生成流程

1. **撰写交底书内容为 JSON**：将专利技术方案按章节组织为 JSON 内容文件，不可写为 Markdown。键为章节名，值为字符串或数组（详见下方命令示例）。
2. **生成图示（可选）**：用 `tools/mermaid_render.py` 将 mermaid 围栏转为 PNG。
3. **填充模板（两种方式）**：

#### 方式一：Python 版（无需 dotnet，推荐快速使用）
...
cat > content.json << 'EOF'
{
  ...
  "附图说明": "附图说明文本...",
  "具体实施方式": "具体实施方式文本...",
  "权利要求书": ["1、一种...方法", "S1、...", "2、根据..."]
}
EOF

# 填充模板（替换内容，保留格式）
python3 tools/minimax-docx/template_fill.py \
  --template "examples/参考示例--一种光盘数据读取过程中的回读信号样本序列检测方法.docx" \
  --content content.json \
  --output "outputs/案件名/案件名_YYYYMMDDHHmmss.docx"
```

#### 方式二：C# OpenXML SDK 版（格式保留更精确，需 dotnet 8.0+）
...
两种方式均以参考示例 DOCX 为模板，**仅替换文字内容，不改变任何格式**（字体、字号、行距、缩进、分节、页脚、图片框架等全部保留）。

**注意**：必须先准备 JSON 内容文件，再执行模板填充命令。不要先生成 Markdown 再转换——直接操作 DOCX 模板。

4. **降级路径（仅 Markdown→DOCX 基础转换）**：
`python3 tools/md_to_docx.py -i "稿.md" -o "稿.docx"`
